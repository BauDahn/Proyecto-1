import os
import sys
import joblib
import pandas as pd
from sklearn.metrics import accuracy_score, classification_report, confusion_matrix, ConfusionMatrixDisplay
import matplotlib.pyplot as plt

# Rutas absolutas calculadas desde la ubicación de este archivo,
# funcionan independientemente del directorio de trabajo.
_SRC  = os.path.dirname(os.path.abspath(__file__))
_RAIZ = os.path.dirname(_SRC)

RUTA_DATOS         = os.path.join(_RAIZ, 'data', 'processed', 'dataset_clean.csv')
RUTA_MODELO_REGRESION = os.path.join(_SRC, 'modelo_regresion.pkl')
RUTA_MODELO_ARBOL     = os.path.join(_SRC, 'modelo_arbol.pkl')


def validate_model(model_path, X_test, y_test):
    # Comprobamos que el archivo existe antes de intentar cargarlo
    if not os.path.exists(model_path):
        print(f"Error: no se encontró el modelo en:\n  {model_path}")
        print("Ejecuta primero el módulo de entrenamiento correspondiente.")
        return None

    # 1. Cargar el modelo .pkl
    model = joblib.load(model_path)

    # 2. Realizar predicciones
    y_pred = model.predict(X_test)

    # 3. Métricas principales
    print("--- Reporte de Clasificación ---")
    print(classification_report(y_test, y_pred))

    # 4. Matriz de Confusión
    cm = confusion_matrix(y_test, y_pred)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm)
    disp.plot()
    plt.title("Matriz de Confusión")
    plt.show()

    return accuracy_score(y_test, y_pred)


if __name__ == '__main__':
    if not os.path.exists(RUTA_DATOS):
        print(f"Error: no se encontró el dataset en:\n  {RUTA_DATOS}")
        sys.exit(1)

    df = pd.read_csv(RUTA_DATOS, sep=';')
    y = df["TTO"]
    X = df.loc[:, df.columns != 'TTO']

    validate_model(RUTA_MODELO_REGRESION, X, y)